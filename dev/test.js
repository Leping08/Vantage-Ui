...
export const tab = {
  ...
  render() {
    return h('div', { class: 'whitespace-no-wrap ml-8 py-4 px-1 font-medium text-sm leading-5' }, h(this.$slots.default))
  }
}
...
export const tabs = {
  ...
  render() {
    ...
    const tabs = elements.filter(elements => elements.type === tab).map(tab => {
      return h(tab, {
        class: tab.props.tabId ===  this.$props.activeTabId ? 
    ...
    const content = elements.find(element => 
      element.type === tabContent &&
      element.props.tabId === this.activeTabId
    )
